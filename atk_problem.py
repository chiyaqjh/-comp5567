...
def sign_input_utxo(input_utxo_id, privkey):
    """签名输入UTXO"""
    # 确保 input_utxo_id 是字节类型
    if isinstance(input_utxo_id, str):
        message = input_utxo_id.encode('utf-8')
    else:
        message = str(input_utxo_id).encode('utf-8')
    
    # 使用 RSA 签名
    signature = rsa.sign(message, privkey, 'SHA-1')
    
    # 将字节签名转换为十六进制字符串（Python 3 方式）
...
url_prefix = "http://127.0.0.1:5000/b9af31f66147e/"
s = requests.session()
...
genesis_blocks = list(filter(lambda i: i['height'] == 0, blocks))
genesis_block = genesis_blocks[0]


# replay attack
#attacked_block = filter(lambda i: i['height'] == 1, blocks)[0]
attacked_block = list(filter(lambda i: i['height'] == 1, blocks))

replayed_tx = attacked_block[0]['transactions'][0]
